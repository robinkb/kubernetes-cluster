...
//timoni:generate timoni vendor crd -f https://github.com/metallb/metallb/raw/main/config/crd/bases/metallb.io_ipaddresspools.yaml
...
#IPAddressPoolSpec: {
	...
	// share the same settings. Each range can be either a CIDR
	...
	// AvoidBuggyIPs prevents addresses ending with .0 and .255 to be
	// used by a pool.
	avoidBuggyIPs?: bool | *false

	// AllocateTo makes ip pool allocation to specific namespace
	// and/or service. The controller will use the pool with lowest
	// value of priority in case of multiple matches. A pool with no
	// priority set will be used only if the pools with priority
	// can't be used. If multiple matching IPAddressPools are
	// available it will check for the availability of IPs sorting
	// the matching IPAddressPools by priority, starting from the
	// highest to the lowest. If multiple IPAddressPools have the
	// same priority, choice will be random.
	serviceAllocation?: {
		...
		// for which ip pool can be used for ip allocation.
		serviceSelectors?: [...{
			// matchExpressions is a list of label selector requirements. The
			// requirements are ANDed.
			matchExpressions?: [...{
